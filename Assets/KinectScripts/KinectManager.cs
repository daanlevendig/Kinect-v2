...
public class KinectManager : MonoBehaviour 
{
	...
	//private bool allowTurnAround = false;
		...
		if(kinectInitialized)
		{
			...
			else if(computeColorMap && displayColorMap)
			{
				if(usersClrRect.width == 0 || usersClrRect.height == 0)
				{
					...
					usersClrRect = new Rect((cameraRect.width/2 - displayWidth/2), cameraRect.height, displayWidth, -displayHeight);
...
//
//				if(allowTurnAround && sensorData.sensorInterface.IsFaceTrackingActive() &&
//				   bodyData.joint[(int)KinectInterop.JointType.Neck].trackingState != KinectInterop.TrackingState.NotTracked)
//				{
//					float bodyTurnAngle = bodyData.bodyTurnAngle > 180f ? bodyData.bodyTurnAngle - 360f : bodyData.bodyTurnAngle;
//					float neckTiltAngle = Vector3.Angle(Vector3.up, bodyData.joint[(int)KinectInterop.JointType.Neck].direction.normalized);
//
...
//							bodyData.turnAroundFactor += 10f * Time.deltaTime;
...
//						//bodyData.isTurnedAround = false; // bTurnedAround;
//						
//						if(bodyData.isTurnedAround)
//						{
//							bodyData.bodyFullAngle = bodyTurnAngle >= 0 ? (180f - bodyTurnAngle) : -(180f - bodyTurnAngle);
//							
...
//							"TurnAround: {0}, Factor: {1:F3}, TurnAngle: {2:000}, FullAngle: {3:000}, NeckAngle: {4:000}", 
//							(bodyData.isTurnedAround ? "Y" : "n"), bodyData.turnAroundFactor, 
//							bodyTurnAngle, bodyData.bodyFullAngle, neckTiltAngle);
//					}
//				}
//
			...
			if(bJointValid)
			{
				...
				if(nextJoint != joint && nextJoint >= 0 && nextJoint < sensorData.jointCount)
				{
					   ...
					   (joint == (int)KinectInterop.JointType.Neck))
					{
						...
						Vector3 jointDir2 = !bodyData.isTurnedAround ? bodyData.shouldersDirection : -bodyData.shouldersDirection;
					...
					{
						Vector3 baseDir2 = Vector3.right;
						Vector3 jointDir2 = !bodyData.isTurnedAround ? bodyData.hipsDirection : -bodyData.hipsDirection;
						jointDir2.z = -jointDir2.z;
